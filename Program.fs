...
// // let allowedEnd   = TimeSpan(22, 30, 0)   // 22:30  (10:30 PM)
// let allowedEnd   = TimeSpan(21, 0, 0)   // 21:00  (9:00 PM)
// // let interval     = 5_000.0                 // Every 5 seconds -- turns out this is too infrequent
...
//             processName.Equals(target, StringComparison.OrdinalIgnoreCase)) then
//       Some { Pid = pid; Name = processName }
//     else
//       None
//   with 
...
//     if now < allowedStart || now >= allowedEnd then
//       minimizeApps()
//   finally
...
// (* Application startup *)
// log (sprintf 
//   "ScheduledAppMinimizer started (minimizing outside %s-%s)"
//   (allowedStart.ToString("hh\\:mm"))
//   (allowedEnd.ToString("hh\\:mm")))
// log (sprintf "Log file location: %s" logFile)
// log (sprintf "Monitoring processes: %s" (String.Join(", ", targetProcesses)))

// // Start the timer
// timer.Start()

// // For headless operation, simply keep the process alive
// // The application will run until terminated by the system or task manager
